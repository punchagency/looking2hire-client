...
class _CreateCandidateAccountScreenState extends State<CreateCandidateAccountScreen> {
  ...
  void validatePasswords(TextEditingController password, TextEditingController cPassword) {
    setState(() {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    CustomIconTextField(
                      ...
                      isPassword: true,
                      onChanged:
                          (value) => validatePasswords(provider.passwordController, provider.confirmPasswordController),
                      validate: (value) {
                        return Validator.validateIsNotEmpty(value);
                      },
                    ),
                    ...
                    Button(
                      onPressed: () async {
                        if (formKey.currentState?.validate() == true) {
                          validatePasswords(provider.passwordController, provider.confirmPasswordController);
                          if (!passwordsMatch) {
                            setSnackBar(context: context, title: "Error", message: "Passwords do not match");
                            return;
                          } else {
                            await provider.createApplicantAccount(context: context).then((success) {
                              if (success) {
